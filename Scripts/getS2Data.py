...
PATCHES_PER_YEAR = 850
PATCH_PX  = 256
RESOLUTION = 10

# Set starting UID (already generated 3220 for UP)
uid = 11162

# Paths
BASE_DIR = pathlib.Path(r"C:\Users\sumit\Documents\GIS\ForestTemp")
DW_FOLDER = BASE_DIR / "westbengal"     # folder with exported DW rasters
OUT_DIR   = pathlib.Path("dataset")
...
# Create lookup of DW raster files per state and year
DW_FILES = {
    state: {y: DW_FOLDER / f"dw_{state}_{y}.tif" for y in YEARS}
    for state in STATES
}
...
for state in STATES:
    for year in YEARS:
        tif_path = DW_FILES[state][year]
        if not tif_path.exists():
            print(f"{tif_path} missing; skipping {state} {year}")
            continue

        with rasterio.open(tif_path) as src:
            for win in tqdm(sample_windows(src, PATCHES_PER_YEAR),
                            desc=f"{state}-{year}", unit="patch"):
                mask = src.read(1, window=win)
                if np.all(mask == 0):
                    continue
                try:
                ...
                np.save(OUT_IMG / f"{uid:06d}.npy", rgbnir)
                np.save(OUT_MSK / f"{uid:06d}.npy", mask)
                uid += 1

print(f"Finished. Total dataset pairs now: {uid}")
